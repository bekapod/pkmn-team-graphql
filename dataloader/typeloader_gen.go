...
type TypeLoaderConfig struct {
	...
	Fetch func(keys []string) ([]*model.Type, []error)
...
func (l *TypeLoader) Load(key string) (*model.Type, error) {
...
func (l *TypeLoader) LoadThunk(key string) func() (*model.Type, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*model.Type, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *TypeLoader) LoadAll(keys []string) ([]*model.Type, []error) {
	results := make([]func() (*model.Type, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	types := make([]*model.Type, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TypeLoader) LoadAllThunk(keys []string) func() ([]*model.Type, []error) {
	results := make([]func() (*model.Type, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *TypeLoader) Prime(key string, value *model.Type) bool {
	l.mu.Lock()
	var found bool
...
func (l *TypeLoader) Clear(key string) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *TypeLoader) unsafeSet(key string, value *model.Type) {
	if l.cache == nil {
		l.cache = map[string]*model.Type{}
	}
	l.cache[key] = value
}
...
func (b *typeLoaderBatch) keyIndex(l *TypeLoader, key string) int {
